...
def approx_xsect(faces_rtz, shape_obj, bound_params, resolution):
    ...
    clean_norm_segment = norm_segment[valid_marks, :]

    rpmin = np.min(clean_norm_segment[:, 0])
    rpmax = np.max(clean_norm_segment[:, 0])

    tpmin = np.min(clean_norm_segment[:, 1])
    tpmax = np.max(clean_norm_segment[:, 1])

    zpmin = np.min(clean_norm_segment[:, 2])
    zpmax = np.max(clean_norm_segment[:, 2])

    print(f"    * 1. Polar data is ready: r in [{rpmin:.3f}, {rpmax:.3f}],", end=" ")
    print(f"theta in [{tpmin:.3f}, {tpmax:.3f}],", end=" ")
    print(f"z in [{zpmin:.3f}, {zpmax:.3f}]; shape = {clean_norm_segment.shape}")

    # 2. Approximate the cross-section outline    
    normrz = clean_norm_segment[:, [0,2]]
