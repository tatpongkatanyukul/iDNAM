...
def approx_xsect(faces_rtz, shape_obj, bound_params, resolution):
    ...
    ids_xs = np.where(rmarks & tmarks)[0]
    segment = faces_rtz[ids_xs, :]
    fb["coverage_ids"] = ids_xs
    fb["eff_ids"] = ids_xs
    print(f"    * Coverage: {len(ids_xs)} faces")
    ...
    for i in range(resolution-1):
        ...
        rpi = rs[i]
        bins.append([rpi, zpmax])

        # The next height counts from the previous bin to its bin.
        rpmin = rpi

    # end for i
    
    # The last one covers only one last bin.
    zlast = zpmax
    mark = (normrz[:,0] >= rs[-2]) & (normrz[:,0] <= rs[-1])
    if np.sum(mark) > 0:
        zlast = np.max(normrz[mark, 1])

    bins.append([rpmax, zlast])
    ...
    N, _ = bins.shape
    verts = np.c_[bins, np.zeros((N,1))]   # (N,3) of [r, z, 0] for x,y,z coordinates
